	...
	if(!empty($levels) ) {
		...
		<ul class="list-unstyled">
			...
			<?php foreach( $levels as $level ) {
				...
				?>
				<li><strong><?php esc_html_e("Level", 'streamtube' );?>:</strong> <?php echo esc_html( $level->name ); ?></li>

				<?php if($level->billing_amount > 0) { ?>
					<li><strong><?php esc_html_e("Membership Fee", 'streamtube' );?>:</strong>
						<?php
							$level = $current_user->membership_level;
							if($current_user->membership_level->cycle_number > 1) {
								printf(__('%s every %d %s.', 'streamtube' ), pmpro_escape_price( pmpro_formatPrice($level->billing_amount) ), $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number));
							} elseif($current_user->membership_level->cycle_number == 1) {
								printf(__('%s per %s.', 'streamtube' ), pmpro_escape_price( pmpro_formatPrice($level->billing_amount) ), pmpro_translate_billing_period($level->cycle_period));
							} else {
			...
			<?php if($level->billing_limit) { ?>
				<li><strong><?php esc_html_e("Duration", 'streamtube' );?>:</strong> <?php echo esc_html( $level->billing_limit . ' ' . sornot( $level->cycle_period, $level->billing_limit ) ); ?></li>
			...
			<?php
				...
				if ( $pmpro_billing_show_payment_method && ! empty( $CardType ) && $has_recurring_levels ) { ?>
					<li><strong><?php esc_html_e( 'Payment Method', 'streamtube' ); ?>: </strong>
						<?php echo esc_html( ucwords( $CardType ) ); ?>
						<?php _e('ending in', 'streamtube' ); ?>
						<?php echo esc_html( last4( get_user_meta( $current_user->ID, 'pmpro_AccountNumber', true ) ) ); ?>.
						<?php _e('Expiration', 'streamtube' );?>: <?php echo esc_html( $ExpirationMonth ); ?>/<?php echo esc_html( $ExpirationYear ); ?>
					</li>
					<?php
				}
			?>
...
<?php if ( $has_recurring_levels ) {
	...
	<?php } elseif ( $show_paypal_link ) { ?>
		<p><?php  _e('Your payment subscription is managed by PayPal. Please <a class="text-body fw-bold" href="http://www.paypal.com">login to PayPal here</a> to update your billing information.', 'streamtube' );?>
		</p>
	<?php } elseif ( $gateway != $default_gateway ) {
		...
		?>
		<p><?php esc_html_e( 'Your billing information cannot be updated at this time.', 'streamtube' ); ?></p>
	<?php } else {
		// Show the default gateway form and allow billing information update.
		?>

		<?php printf(
			'<div id="pmpro_level-%s" class="%s">',
			esc_attr( $level->id ),
			pmpro_get_element_class( $pmpro_billing_gateway_class, 'pmpro_level-' . $level->id )
		);?>
		...
		<form id="pmpro_form" class="<?php echo pmpro_get_element_class( 'pmpro_form' ); ?>" action="<?php echo esc_url( pmpro_url( "billing", "", "https") ) ?>" method="post">
			...
			<div id="pmpro_billing_address_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_billing_address_fields' ); ?>">
				<hr />
				<h3>
					<span class="<?php echo pmpro_get_element_class( 'pmpro_checkout-h3-name' ); ?>"><?php esc_html_e('Billing Address', 'streamtube' );?></span>
				</h3>
				<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-fields' ); ?>">
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bfirstname', 'pmpro_checkout-field-bfirstname' ); ?>">
						<label for="bfirstname"><?php esc_html_e('First Name', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-blastname', 'pmpro_checkout-field-blastname' ); ?>">
						<label for="blastname"><?php esc_html_e('Last Name', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-baddress1', 'pmpro_checkout-field-baddress1' ); ?>">
						<label for="baddress1"><?php esc_html_e('Address 1', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-baddress2', 'pmpro_checkout-field-baddress2' ); ?>">
						<label for="baddress2"><?php esc_html_e('Address 2', 'streamtube' );?></label>
						<input id="baddress2" name="baddress2" type="text" class="<?php echo pmpro_get_element_class( 'input', 'baddress2' ); ?>" size="30" value="<?php echo esc_attr($baddress2);?>" /> <small class="<?php echo pmpro_get_element_class( 'lite' ); ?>">(<?php esc_html_e('optional', 'streamtube' );?>)</small>
					...
					<?php
						...
						?>
							<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bcity', 'pmpro_checkout-field-bcity' ); ?>">
								<label for="bcity"><?php esc_html_e('City', 'streamtube' );?></label>
							...
							<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bstate', 'pmpro_checkout-field-bstate' ); ?>">
								<label for="bstate"><?php esc_html_e('State', 'streamtube' );?></label>
							...
							<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bzipcode', 'pmpro_checkout-field-bzipcode' ); ?>">
								<label for="bzipcode"><?php esc_html_e('Postal Code', 'streamtube' );?></label>
						...
						?>
							<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bcity_state_zip', 'pmpro_checkout-field-bcity_state_zip' ); ?>">
								<label for="bcity_state_zip"><?php esc_html_e('City, State Zip', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bcountry', 'pmpro_checkout-field-bcountry' ); ?>">
						<label for="bcountry"><?php esc_html_e('Country', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bphone', 'pmpro_checkout-field-bphone' ); ?>">
						<label for="bphone"><?php esc_html_e('Phone', 'streamtube' );?></label>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bemail', 'pmpro_checkout-field-bemail' ); ?>">
						<label for="bemail"><?php esc_html_e('Email Address', 'streamtube' );?></label>

						<?php printf(
							'<input id="bemail" name="bemail" type="%s" class="%s" size="30" value="%s"/>',
							$pmpro_email_field_type ? 'email' : 'text',
							pmpro_get_element_class( 'input', 'bemail' ),
							esc_attr($bemail)
						);?>
					...
					<?php printf(
						'<div class="%s">',
						pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bconfirmemail', 'pmpro_checkout-field-bconfirmemail' )
					)?>

						<label for="bconfirmemail"><?php esc_html_e('Confirm Email', 'streamtube' );?></label>

						<?php printf(
							'<input id="bconfirmemail" name="bconfirmemail" type="%s" class="%s" size="30" value="%s">',
							$pmpro_email_field_type ? 'email' : 'text',
							pmpro_get_element_class( 'input', 'bconfirmemail' ),
							esc_attr($bconfirmemail)
						)?>						
			...
			if($pmpro_include_payment_information_fields)
			{
				...
				<div id="pmpro_payment_information_fields" class="<?php echo pmpro_get_element_class( 'pmpro_checkout', 'pmpro_payment_information_fields' ); ?>">
					<h3>
						<span class="<?php echo pmpro_get_element_class( 'pmpro_checkout-h3-name' ); ?>"><?php esc_html_e('Credit Card Information', 'streamtube' );?></span>
						<span class="<?php echo pmpro_get_element_class( 'pmpro_checkout-h3-msg' ); ?>"><?php printf( esc_html__('We accept %s', 'streamtube' ), $pmpro_accepted_credit_cards_string ); ?></span>
					...
					<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-fields' ); ?>">
						<?php
							...
							if($pmpro_include_cardtype_field) { ?>
								<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_payment-card-type', 'pmpro_payment-card-type' ); ?>">
									<label for="CardType"><?php esc_html_e('Card Type', 'streamtube' );?></label>
						...
						<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_payment-account-number', 'pmpro_payment-account-number' ); ?>">
							<label for="AccountNumber"><?php esc_html_e('Card Number', 'streamtube' );?></label>
						...
						<div class="<?php echo pmpro_get_element_class( 'pmpro_checkout-field pmpro_payment-expiration', 'pmpro_payment-expiration' ); ?>">
							<label for="ExpirationMonth"><?php esc_html_e('Expiration Date', 'streamtube' );?></label>
			...
			<div class="<?php echo pmpro_get_element_class( 'pmpro_submit' ); ?>">
				...
				<input type="submit" id="pmpro_btn-submit" class="<?php echo pmpro_get_element_class( 'pmpro_btn pmpro_btn-submit-checkout', 'pmpro_btn-submit-checkout' ); ?>" value="<?php esc_attr_e('Update', 'streamtube' );?>" />

				<input type="button" name="cancel" class="<?php echo pmpro_get_element_class( 'pmpro_btn pmpro_btn-cancel', 'pmpro_btn-cancel' ); ?>" value="<?php esc_attr_e('Cancel', 'streamtube' );?>" onclick="location.href='<?php echo pmpro_url("account")?>';" />
			</div>
		</form>
		<script>
			<!--
...
<?php } else { // End for recurring level check.
	...
	if ( isset( $order->membership_id ) && ! empty( $order->membership_id ) && empty( $level->id ) ) {
		...
		if ( ! empty( $level ) && ! empty( $level->allow_signups ) ) {
			...
			printf( __( "Your membership is not active. <a href='%s'>Renew now.</a>", 'streamtube' ), $url );
		} elseif ( ! empty( $default_level_id ) ) {
			$url = pmpro_url( 'checkout', '?level=' . $default_level_id );
			printf( __( "You do not have an active membership. <a href='%s'>Register here.</a>", 'streamtube' ), $url );
		} else {
			$url = pmpro_url( 'levels' );
			printf( __( "You do not have an active membership. <a href='%s'>Choose a membership level.</a>", 'streamtube' ), $url );
		}
	} else { ?>
		<p><?php esc_html_e("This subscription is not recurring. So you don't need to update your billing information.", 'streamtube' );?></p>
	<?php }
} ?>
