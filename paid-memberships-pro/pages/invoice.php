...
<div class="<?php echo pmpro_get_element_class( 'pmpro_invoice_wrap' ); ?>">
	...
	if( $pmpro_invoice ):
		...
		printf(
			'<h3>%s</h3>',
			sprintf(
				esc_html__('Invoice #%s on %s', 'streamtube' ), 
				$pmpro_invoice->code, 
		...
		<ul>
			...
			<?php if($pmpro_invoice->getDiscountCode()) : ?>
				<li>
					<strong><?php esc_html_e('Discount Code', 'streamtube' );?>:</strong>
		...
		<?php printf( '<div class="%s">', esc_attr( pmpro_get_element_class( 'pmpro_invoice_details' ) ) );?>
			...
			<?php if( ! empty( $pmpro_invoice->billing->street ) ) : ?>
				<div class="<?php echo pmpro_get_element_class( 'pmpro_invoice-billing-address' ); ?>">
					<strong><?php esc_html_e('Billing Address', 'streamtube' );?></strong>
					<p>
			...
			<?php if ( ! empty( $pmpro_invoice->accountnumber ) || ! empty( $pmpro_invoice->payment_type ) ) : ?>
				...
				<?php printf( '<div class="%s">', esc_attr( pmpro_get_element_class( 'pmpro_invoice-payment-method' ) ) ); ?>
					...
					<strong><?php esc_html_e('Payment Method', 'streamtube' );?></strong>

					<?php if($pmpro_invoice->accountnumber) : ?>
						<p><?php echo ucwords( $pmpro_invoice->cardtype ); ?> <?php _e('ending in', 'streamtube' );?> <?php echo last4($pmpro_invoice->accountnumber)?>
							<br />

							<?php printf(
								'%s: %s / %s',
								esc_html__('Expiration', 'streamtube' ),
			...
			<?php printf( '<div class="%s">', esc_attr( pmpro_get_element_class( 'pmpro_invoice-total' ) ) );?>
				<strong><?php esc_html_e('Total Billed', 'streamtube' );?></strong>
				<p>
					<?php
		...
		if($invoices)
		{
			...
			<thead>
				<tr>
					<th><?php esc_html_e('Date', 'streamtube' ); ?></th>
					<th><?php esc_html_e('Invoice #', 'streamtube' ); ?></th>
					<th><?php esc_html_e('Level', 'streamtube' ); ?></th>
					<th><?php esc_html_e('Total Billed', 'streamtube' ); ?></th>
				</tr>
			</thead>
			<tbody>
			<?php
		...
		{
			?>
			<p><?php esc_html_e( 'No invoices found.', 'streamtube' );?></p>
			<?php
		}
	endif;
?>
